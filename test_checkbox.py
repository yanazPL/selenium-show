...
def click_checkbox(driver, *checbox_names):
    for name in checbox_names:
        checkbox = driver.find_element(
            By.XPATH, f'//span[@class="rct-title" and text()="{name}"]'
        )
...
def assert_names_in_results(driver, *names):
    for name in names:
        result_span = driver.find_element(
            By.XPATH, f'//span[@class="text-success" and text()="{name}"]'
        )
...
def test_expand_one_by_one(selenium):
    ...
    collapse_icon = selenium.find_element(By.CLASS_NAME, "rct-option-collapse-all")
    collapse_icon.click()
    while expand_icons := selenium.find_elements(
        By.CLASS_NAME, "rct-icon-expand-close"
    ):
